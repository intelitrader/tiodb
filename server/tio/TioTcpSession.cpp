...
namespace tio
{
	...
	TioTcpSession::TioTcpSession(asio::io_service& io_service, TioTcpServer& server, unsigned int id) :
		...
		local_socket_(io_service),
#endif // BOOST_ASIO_HAS_LOCAL_SOCKETS
		isRemoteSocket_(true),
		server_(server),
		lastHandle_(0),
		valid_(true),
		pendingSendSize_(0),
	...
	TioTcpSession::socket_t& TioTcpSession::GetSocket()
	{
		return socket_;
	}

#if BOOST_ASIO_HAS_LOCAL_SOCKETS
	TioTcpSession::local_socket_t& TioTcpSession::GetLocalSocket()
	{
		return local_socket_;
	}
#endif // BOOST_ASIO_HAS_LOCAL_SOCKETS

	void TioTcpSession::OnAccept(bool isRemoteSocket)
	{
		isRemoteSocket_ = isRemoteSocket;
	...
	{
		tio_lock_guard lock(bigLock_);

		if (buf_.size())
		{
			OnReadHttpCommand(httpParser, error_code(), buf_.size());
		}
		else if( isRemoteSocket_ )
		{
		...
		{
			asio::async_read(local_socket_, buf_,
				wrap_callback(
					[shared_this = shared_from_this(), httpParser](const error_code& err, size_t read)
			{
				shared_this->OnReadHttpCommand(httpParser, err, read);
	...
	void TioTcpSession::ReadCommand()
	{
		...
		{
			asio::async_read_until(local_socket_, buf_, '\n',
				wrap_callback(
					[shared_this](const error_code& err, size_t read)
					{
						shared_this->OnReadCommand(err, read);
	...
	void TioTcpSession::OnReadCommand(const error_code& err, size_t read)
	{
		...
		if (moreDataSize)
		{
			...
			else
			{
				...
				{
					asio::async_read(
						local_socket_, buf_, asio::transfer_at_least(moreDataSize - buf_.size()),
						wrap_callback(
							[shared_this, moreDataSize](const error_code& err, size_t read)
							{
								shared_this->OnCommandData(moreDataSize, err, read);
	...
	void TioTcpSession::SendTextSubscriptionSnapshot(const shared_ptr<ITioResultSet>& resultSet, unsigned int handle, ContainerEventCode eventCode)
	{
		...
		SendTextEvent(handle, EVENT_CODE_SNAPSHOT_END, nullptr, nullptr, nullptr);
	}
	...
	void TioTcpSession::SendBinarySubscriptionSnapshot(const shared_ptr<ITioResultSet>& resultSet, unsigned int handle, ContainerEventCode eventCode)
	{
		...
		auto snapshotEndEvent = Pr1CreateMessage();

		Pr1MessageAddField(snapshotEndEvent.get(), MESSAGE_FIELD_ID_COMMAND, TIO_COMMAND_EVENT);
		Pr1MessageAddField(snapshotEndEvent.get(), MESSAGE_FIELD_ID_HANDLE, handle);
		Pr1MessageAddField(snapshotEndEvent.get(), MESSAGE_FIELD_ID_EVENT_CODE, TIO_EVENT_SNAPSHOT_END);

		messages.push_back(snapshotEndEvent);
	...
	{
		{
			std::unique_lock<std::recursive_mutex> lock(writeLock_);
	...
	unsigned int TioTcpSession::RegisterContainer(const string& containerName, shared_ptr<ITioContainer> container)
	{
		...
		auto i = containerNameToHandle_.find(containerName);

		if(i != containerNameToHandle_.end())
			return i->second;

		unsigned int handle = ++lastHandle_;
		handles_.emplace(make_pair(handle, HandleInfo(container)));
		containerNameToHandle_[containerName] = handle;

		return handle;
	}

	unsigned int TioTcpSession::RegisterNewGroupContainer(const string& groupName, const string& containerName, shared_ptr<ITioContainer> container)
	{
		auto handle = RegisterContainer(containerName, container);
	...
	void TioTcpSession::CloseContainerHandle(unsigned int handle)
	{
		...
		string& containerName = i->second.container->GetName();

		auto ii = containerNameToHandle_.find(containerName);
		if(ii != containerNameToHandle_.end())
			containerNameToHandle_.erase(ii);
	...
	void TioTcpSession::PublishNewGroupContainer(
		const string& groupName, unsigned handle, 
		const shared_ptr<ITioContainer>& container)
	{
		if(binaryProtocol_)
		{
			auto newGroupContainerMessage = Pr1CreateMessage();

			Pr1MessageAddField(newGroupContainerMessage.get(), MESSAGE_FIELD_ID_COMMAND, TIO_COMMAND_NEW_GROUP_CONTAINER);
			Pr1MessageAddField(newGroupContainerMessage.get(), MESSAGE_FIELD_ID_HANDLE, handle);
			Pr1MessageAddField(newGroupContainerMessage.get(), MESSAGE_FIELD_ID_GROUP_NAME, groupName);
			Pr1MessageAddField(newGroupContainerMessage.get(), MESSAGE_FIELD_ID_CONTAINER_NAME, container->GetName());
			Pr1MessageAddField(newGroupContainerMessage.get(), MESSAGE_FIELD_ID_CONTAINER_TYPE, container->GetType());
			
			SendBinaryMessage(newGroupContainerMessage);
		}
		else
		{
	...
	void TioTcpSession::SendPendingBinaryData()
	{
		...
		while(!pendingBinarySendData_.empty())
		{
			...
			pr1_message_get_buffer(item.get(), &buffer, &bufferSize, ++debugSeqNum_);
